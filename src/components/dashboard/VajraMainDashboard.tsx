...
// Specialized AI Analysis View matching the requested blueprint
import { VajraAiAnalysisView } from '../analytics/VajraAiAnalysisView';
import { VajraReportsView } from '../reports/VajraReportsView';
import { VajraResourcesView } from '../resources/VajraResourcesView';
import { VajraThreatsView } from '../threats/VajraThreatsView';
import { BharatDetailPanel } from '../command/BharatDetailPanel';
import { AlertCenterWidget } from '../alerts/AlertCenterWidget';
import { PersonnelTelemetryWidget } from '../telemetry/PersonnelTelemetryWidget';
import { AuditTrailWidget } from '../audit/AuditTrailWidget';
...
export const VajraMainDashboard: React.FC<VajraMainDashboardProps> = ({ user, onLogout }) => {
  // Default to 'DASHBOARD' so dashboard is shown immediately after login
  const [activeNav, setActiveNav] = useState<'DASHBOARD' | 'FEEDS' | 'THREATS' | 'AI' | 'REPORTS' | 'RESOURCES' | 'SETTINGS'>('DASHBOARD');
  const [liveClock, setLiveClock] = useState<string>('21:47:32 IST');
  const [liveDate, setLiveDate] = useState<string>('Sat, 20 Sept 2026');
  ...
  useEffect(() => {
    const updateTime = () => {
      ...
      setLiveClock(`${timePart} IST`);
      setLiveDate(datePart);
    };
    updateTime();
  ...
  return (
    <div className="relative w-full h-full min-h-screen bg-[#010614] text-slate-100 flex flex-col overflow-x-hidden font-tech select-none" id="vajra-main-dashboard">
      ...
      <header className="relative z-30 w-full h-16 bg-[#02091b]/95 border-b border-[#0055ff]/40 px-3 sm:px-6 flex items-center justify-between backdrop-blur-xl shrink-0">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="relative">
            ...
            >
              <div className="w-8 h-8 rounded-lg bg-cyan-900/60 border border-cyan-400/60 flex items-center justify-center overflow-hidden shrink-0">
      ...
      <div className="relative flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 flex flex-col p-3 sm:p-4 overflow-y-auto space-y-3.5">
          ...
          {activeNav === 'FEEDS' && (
            <div className="relative w-full flex-1 rounded-2xl border border-cyan-500/30 p-4 bg-slate-950/90 space-y-4">
              <div className="flex items-center justify-between pb-3 border-b border-cyan-500/20">
                <h2 className="text-base font-tech font-bold text-white flex items-center gap-2">
                  <Camera className="w-5 h-5 text-cyan-400" />
                  LIVE SURVEILLANCE & RECONNAISSANCE FEEDS
                </h2>
                <button
                  onClick={() => setActiveNav('DASHBOARD')}
                  className="px-3 py-1 bg-cyan-950/80 border border-cyan-500/40 rounded-lg text-xs font-mono-code text-cyan-300 hover:text-white transition cursor-pointer"
                >
                  Return to Dashboard
                </button>
              </div>
          ...
          {activeNav === 'SETTINGS' && (
            <div className="relative w-full flex-1 rounded-2xl border border-cyan-500/30 p-4 bg-slate-950/90">
              ...
              <AuditTrailWidget onClose={() => setActiveNav('DASHBOARD')} />
            </div>
          )}

          {/* ======================= BOTTOM ECG / PULSE TACTICAL FOOTER ======================= */}
          <footer className="mt-auto py-2.5 px-4 rounded-xl bg-[#020b1c]/90 border border-[#0055ff]/35 text-center flex items-center justify-center gap-4 backdrop-blur-md">
            ...
            <span className="text-[11px] sm:text-xs font-tech font-bold tracking-[0.25em] text-cyan-400 uppercase">
              VAJRA • AI ANALYTICS • SAFER BORDERS • STRONGER NATION
            </span>
