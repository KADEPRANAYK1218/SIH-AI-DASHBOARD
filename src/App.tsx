...
import { VajraGeocoreMap } from './components/maps/VajraGeocoreMap';
import { CommandOrbit, ActiveCommandTab } from './components/navigation/CommandOrbit';
import { PersonnelTelemetryWidget } from './components/telemetry/PersonnelTelemetryWidget';
import { EnvironmentWidget } from './components/command/EnvironmentWidget';
import { AlertCenterWidget } from './components/alerts/AlertCenterWidget';
import { AnalyticsDashboardWidget } from './components/analytics/AnalyticsDashboardWidget';
import { DecisionSupportWidget } from './components/xai/DecisionSupportWidget';
import { AuditTrailWidget } from './components/audit/AuditTrailWidget';
import { BharatDetailPanel } from './components/command/BharatDetailPanel';
import { SituationalOverviewWidget } from './components/command/SituationalOverviewWidget';
...
export default function App() {
  // Authentication & Session State - Starts at LOGIN first as requested
  ...
  // Dashboard state
  const [activeCommandTab, setActiveCommandTab] = useState<ActiveCommandTab>('MAP_OVERVIEW');
  const [selectedLocation, setSelectedLocation] = useState<GeoLocationItem | null>(null);
  const [simulationActive, setSimulationActive] = useState<boolean>(false);

  // Map layer filters
  const [activeLayers, setActiveLayers] = useState({
    strategicCommands: true,
    airBases: true,
    navalDockyards: true,
    radarsAndSensors: true,
    tacticalAlerts: true,
    weatherOverlay: true,
    radarRings: true
  });
  ...
  // Handler: Credentials verified from login form -> Compulsory Hardware Permission Check
  const handleCredentialsVerified = (
    ...
    // Compulsory Camera & Microphone Hardware activation
    if (!mediaStream) {
  ...
  // Biometric Step 3: Voice Success
  const handleVoiceSuccess = () => {
    setAuthFactors(prev => ({ ...prev, voice: true }));
    setAuthStage('VERIFICATION_CINEMATIC');
  };
  ...
  // Final Cinematic Complete -> Transition to Dashboard
  ...
  const handleLogout = () => {
    ...
    setAuthStage('LOGIN');
    setActiveCommandTab('MAP_OVERVIEW');
  };
